...
async function main() {
  ...
  if (state.lastSession === sessionDate) {
  ...
  if (!noTrade) {
    for (const u of universe) {
      ...
      if (fivePct) sc2 += 2;
      if (hot) sc2 += 4;
      sc2 += Math.min(3, (tv / 1e9));          // liquidity bonus
  ...
  // --- journal update (paper portfolio) ---
  ...
  console.log(`Done. Session ${sessionDate} | gear ${gear} | picks ${picks.length} | equity ₹${state.equity}`);
}
